...
```{r}
# Piechart
p_pie <- ggplot(data = bike05, aes(x = "", y = n, fill = as.factor(YEAR))) +
...
```{r out.width="90%", fig.width=7.9}
(p_bar + p_pie) + plot_annotation(tag_levels = "a")
```
...
a.   An e-bike with a range of 85 kilometers.

b.   A report from a different product recommendation service that has also gathered data from a different, but also representative sample of 15 e-bikes from this same manufacturer, with a mean range of 85 kilometers.
...
The following three items were reworked into one context, as the current [Movie Wrangling](instrument.qmd#sec-movie-wrangling) (pseudocode; column-wise operations; joins).
...
1. Consider the two following sets of pseudocode (ie. code recipe). Would they produce the same results? Explain.
...
<!-- </style> -->
<!-- ``` -->
2. Consider the two following sets of pseudocode (ie. code recipe). Would they produce the same results? Explain.
